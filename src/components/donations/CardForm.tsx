...
export function CardForm({ setMode, card, customerId, updatedFunction, handleDelete, isKingdomFunding, kfTokenizationKey, kfSandbox }: Props) {
  ...
  const createCardKF = async () => {
    ...
    try {
      ...
      console.log("[KF AddCard] Payload:", JSON.stringify(paymentMethod));
      const result = await ApiHelper.post("/paymentmethods/addcard", paymentMethod, "GivingApi");
      console.log("[KF AddCard] Result:", JSON.stringify(result));
      if (result?.error) {
      ...
      console.error("[KF AddCard] Error:", err);
    ...
    return (
      <Card style={{ marginBottom: spacing.md }}>
        ...
        <Card.Content>
          ...
          <Text variant="bodySmall" style={{ marginBottom: spacing.md, color: "#666" }}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: spacing.md }}>
            ...
            <Button mode="contained" onPress={handleDelete} buttonColor="#d32f2f" textColor="#FFFFFF" style={{ flex: 1, marginLeft: spacing.sm }}>
              {t("common.delete")}
            </Button>
          </View>
        </Card.Content>
